...
- **Fix pass (PR #53 follow-up — same branch):** three voting defects.
  ...
  - *Vote-icon UX.* Active vs inactive now reads as filled vs outline (enclosed triangle icons + `fill` toggle), and the title/aria-label flips to "Remove vote" when active so the un-vote toggle is discoverable. Added `gallery.vote_remove` (en/es) + a `translate()` helper in `i18nClient.ts` to keep the swapped labels localized.
  - Tests: added 2 regression tests to `test_lib_votes.py` (service rolls back + re-raises on `IntegrityError`; `POST /votes` maps it to 409) — `test_lib_votes.py` is 13/13 green. Verified live (vote/toggle/flip/clear → HTTP 200, no 500). `bun run build` clean. Note: `python:3.14-slim` / `requires-python>=3.14` has no working `pydantic_core` wheel on arm64, so the API container won't boot after a rebuild and in-container `make pytest` can't run on this host; tests + live API were verified on a host Python 3.13 venv against the Docker DB. Pin kept at 3.14 per team direction.
- Files affected: `api/app/lib/internal/models.py`, `api/app/lib/internal/actions_service.py` (new), `api/app/lib/routes/actions.py`, `api/tests/test_lib.py`, `api/tests/test_lib_votes.py` (new), `ui/src/types/api.ts`, `ui/src/lib/actions.ts` (new), `ui/src/lib/toast.ts` (new), `ui/src/lib/catalogGallery.ts`, `ui/src/lib/catalogDetail.ts`, `ui/src/utils/i18nClient.ts`, `ui/src/components/lib/gallery/GalleryCard.astro`, `ui/src/components/lib/gallery/CatalogDetailModal.astro`, `ui/src/components/lib/{PromptCard,AgentCard,McpCard}.astro`, `ui/src/pages/lib/{prompts,agents,mcps}.astro`, `ui/src/i18n/{en,es}.json`, `docs/user-stories/lib-assets-roadmap.md` (new)
