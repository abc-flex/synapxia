...
## 2026-07-02 17:19 — feat(lib): Review page (HU-Review) + redirect back to the gallery after Propose
...
- **Redirect after Propose.** When a Propose was started from a gallery's Propose CTA (`/lib/propose?category=…`, so the type is locked), a successful submit now **redirects back to that gallery** (`/lib/{category}?proposed=1`) and shows a "your asset has been proposed" toast there (via `catalogGallery.ts`, which strips the `?proposed` param so a refresh doesn't re-toast). Direct visits (no locked category) keep the in-place confirm + reset.
- **Feedback rules:** required for Reject + Request changes (proposer needs the reason), optional for Approve.
- **Out of scope:** HU-Modify (the proposer editing after "request changes") stays future — the `MODIFICATION` notification still shows "coming soon" in the bell until a `/lib/modify` page exists.
- **Verify:** `api/tests/test_lib_review.py` — 10 tests pass on the 3.12 venv (status flip + `REVIEW/FINISHED` + proposer notification for approve/reject/changes, blank-feedback→None, non-assigned/ineligible→403, already-decided→409, invalid decision→400, route 200/403); full lib suite 114 passed. `bun run build` clean (new `review` chunk 6.65 kB / 3.0 kB gz); `tsc` + `svelte-check` show only the pre-existing `advancedTable.ts` errors. **Deploy:** no schema change — enums (ASSET_STATUS/ACTION_TYPE/WORKFLOW_STATUS) already seeded; the REVIEWER profile ships from PR #69.
- Files affected: `api/app/lib/internal/review_service.py` (new), `api/app/lib/internal/models.py`, `api/app/lib/routes/assets.py`, `api/tests/test_lib_review.py` (new), `ui/src/lib/review.ts` (new), `ui/src/types/api.ts`, `ui/src/pages/lib/review.astro` (new), `ui/src/components/svelte/ReviewAction.svelte` (new), `ui/src/components/svelte/NotificationBell.svelte`, `ui/src/pages/lib/propose.astro`, `ui/src/lib/catalogGallery.ts`, `ui/src/i18n/en.json`, `ui/src/i18n/es.json`, `memory/MEMORY.md`
