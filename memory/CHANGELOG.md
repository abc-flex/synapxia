...
## 2026-07-01 15:41 — feat(ui): adopt Svelte 5 for interactive islands (pilot: Show Action), on Vite 8
...
- **Pilot:** reimplemented **`/lib/show-action`** (HU-Show Action) as a Svelte component `ui/src/components/svelte/ShowAction.svelte`, **mounted manually** via Svelte 5's `mount()` from a bundled `<script>` in `show-action.astro`. Same UX/behaviour as the vanilla version; reuses the existing `lib/*` services (`getAction`/`getAsset`/`markNotified`/`dismissNotification`) + `translate()` i18n — no backend change.
...
- **Not done (deliberately):** `bun update --latest` pulls Astro 7 (Rolldown, RC-grade) + Tailwind 4 + Vercel 11 and still doesn't fix Svelte — reverted; we stay on Astro 5.x. Bigger island migrations (DataTable, catalog detail, notifications bell) are follow-ups now that the toolchain is proven.
- **Verify:** `bun run build` clean (Svelte bundle emits, 33.5 kB); `tsc` + `svelte-check` show only the pre-existing `advancedTable.ts` errors.
- Files affected: `ui/package.json`, `ui/astro.config.mjs`, `ui/svelte.config.js` (new), `ui/tailwind.config.cjs`, `ui/src/components/svelte/ShowAction.svelte` (new), `ui/src/pages/lib/show-action.astro`, `ui/CLAUDE.md`, `memory/MEMORY.md`
