...
- **Header (mobile, matches Figma mockup)**: single compact row — `toggle · brand · search · controls`. The brand (logo + "SynapxIA") shows on the left on phones; the right side carries **only the bell + account** (matching the mockup), with theme + language switchers moved to `sm+` (tablet/desktop) so the phone bar stays clean and never overflows. Account first-name hidden <sm; avatar margin trimmed <sm.
