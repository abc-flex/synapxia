...
## 2026-07-12 16:43 — feat(lib): asset versioning on edit/save (HU-LI09, versioning half)

- **Editing + saving an existing asset in the repo now creates a NEW VERSION instead of a silent in-place update.** Same-asset-row model (per the columns commit 496980c introduced): `assets.current_version` is bumped and the characterizations are snapshotted under the new `characterizations.version_label` (part of the PK `(asset, version_label, feature)`) — prior generations stay stored untouched as history. The user picks the change type at save time (Major / Minor / Patch radio pills in the modal footer, edit mode only) and the matching digit of `X.Y.Z` bumps (major → X+1.0.0, minor → X.Y+1.0, patch → X.Y.Z+1). Each save logs one `VERSIONING`/`FINISHED` action — the history timeline already localizes it as "created a new version", so the Activity tab shows versions with zero extra UI.
...
- **Frontend:** `AssetDetailModal.astro` shows a `vX.Y.Z` badge next to the title in edit mode and the change-type picker in the footer; its edit-mode save now makes ONE `createAssetVersion` call (core fields when the section includes core; the tabs' full characterization snapshot via new `AssetDetailTabs.charSnapshot()` when it includes detail), then flushes relations/permissions (not versioned) via `tabs.flush(id, {skipChars:true})`. Create mode byte-identical. `/lib/assets` table gained a Version column. New i18n keys (`asset_modal.version`, `asset_detail_modal.version_saved`/`save_new_version`/`current_version`/`change_type_*`) in both `en.json` and `es.json`.
- **Versioning UX (follow-up commit on the same branch):** in edit mode the submit button now reads **"Save new version"** (restored to "Save" for create in `resetForm`), and under the Major/Minor/Patch pills a **live preview** renders `v1.2.0 → v1.3.0` — old label struck-through/muted, arrow, new label with **the bumping digit highlighted** (indigo chip; major→1st, minor→2nd, patch→3rd) — recomputed on every pill change via a new client-side `bumpVersionLabel()` in `ui/src/lib/assets.ts` mirroring the backend's `bump_label` (display-only; the server stays authoritative).
