...
- **Mobile header search**: below `sm` the input collapses to an icon-only button; tapping it expands into a fixed floating card pinned below the header (`.search-palette-overlay`, mirrors `.notification-dropdown`'s mobile pattern) showing the same command-palette input + results. Desktop unchanged.
