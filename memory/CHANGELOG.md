...
## 2026-07-01 16:33 — feat(ui): adopt Svelte 5 for interactive islands (Show Action + Notifications bell + Discussion), on Vite 8
...
- **Not done (deliberately):** `bun update --latest` pulls Astro 7 (Rolldown, RC-grade) + Tailwind 4 + Vercel 11 and still doesn't fix Svelte — reverted; we stay on Astro 5.x. Bigger island migrations (DataTable, catalog detail modal) are follow-ups now that the toolchain is proven.
- **Verify:** `bun run build` clean (Foro island chunk emits, 10.1 kB / 3.9 kB gz); `tsc` + `svelte-check` show only the pre-existing `advancedTable.ts` errors + a `tsconfig baseUrl` deprecation warning (`Foro.svelte` itself: 0 errors, 0 warnings).
- Files affected: `ui/package.json`, `ui/astro.config.mjs`, `ui/svelte.config.js` (new), `ui/tailwind.config.cjs`, `ui/src/components/svelte/ShowAction.svelte` (new), `ui/src/components/svelte/NotificationBell.svelte` (new), `ui/src/components/svelte/Foro.svelte` (new), `ui/src/pages/lib/show-action.astro`, `ui/src/components/core/header/NotificationMenu.astro`, `ui/src/components/lib/gallery/Foro.astro`, `ui/src/lib/notifications.ts`, `ui/src/lib/foro.ts`, `ui/src/lib/catalogDetail.ts`, `ui/CLAUDE.md`, `memory/MEMORY.md`
