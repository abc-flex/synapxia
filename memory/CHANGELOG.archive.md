# Changelog Archive (pre-2026-06-10)

Historical entries from before the 2026-06-10 switchover to the hand-written
`## YYYY-MM-DD HH:MM — Title` changelog format (see the "Adopt human-written CHANGELOG
flow" entry at the top of `memory/CHANGELOG.md`). These use the old Keep-a-Changelog–style
`### Added / Changed / Fixed` blocks auto-generated by a now-removed git-hook flow
(`.githooks/post-commit` + `post-merge`). Kept as-is for traceability — **never edit**,
same rule as the live changelog. Newest at top, oldest (`[0.0.1]` initial setup) at bottom.
