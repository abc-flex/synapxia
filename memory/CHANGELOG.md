...
- **Mobile header search**: below `sm` the input collapses to an icon-only button; tapping it expands into a fixed floating card pinned below the header (`.search-palette-overlay`, mirrors `.notification-dropdown`'s mobile pattern) showing the same command-palette input + results. Desktop unchanged. **Follow-up fix:** collapsing `SearchBar`'s host to `shrink-0` below `sm` left nothing to flex-fill the row there, bunching hamburger/logo/search/bell/account together on the left with a dead gap on the right — `Header.astro`'s spacer div is now `flex-1 sm:hidden lg:block` (active below `sm` too, not just from `lg`) so the bell/account cluster sits flush against the right edge on phones again.
