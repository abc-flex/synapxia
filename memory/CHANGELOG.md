...
- **Follow-ups (same PR):**
  - **Read-only Discussion / History / Versions tabs on the edit modal** — the editable modal (`AssetDetailTabs.svelte`) now also surfaces the three view-only tabs (only for an existing asset), reusing the gallery controllers: `<Foro>` nested directly, `mountHistory` + `mountVersions` (with a new all-category `ALL_DETAIL_SECTIONS` in `lib/assetSections.ts`) wired from `AssetDetailModal.astro`. On those tabs `paintFooterForTab` hides the change-type picker **and** the Save button (view-only; Cancel/close remains).
  - **Labels:** "Characterizations" tab → singular "Characterization"; the Related tab's save button → "Save related assets"; sidebar `menu_options.assets` "Asset Repository" → **"Assets Management"**; `menu_options.agents` "Agent Index" → **"Agents repository"** (both i18n + `db/sql/12-admin-insert.sql` seed `options.name`; agents page title literals too).
  - **Footer compaction:** the change-type footer is now 2 rows (legend + pills/preview on one row) instead of 3.
  - **Default change type is patch end-to-end:** the FE already defaulted to patch; `VersionRequest.change_type` (`api/app/lib/internal/models.py`) now also `Field(default="patch")` so the API defaults to a patch bump when the client omits it (new test in `api/tests/test_lib_versioning.py`, 23 pass on venv312).
  - **Tab-behavior matrix reconciliation:** per the user's final matrix, Permissions is editable on the Asset Management (edit) modal but **"Not applicable" on the Explore/gallery view modal** — so the read-only Permissions tab that was briefly added to `CatalogDetailModal` was removed (permissions stay a management concern, off the public browse surface). The `GET /api/asset_permissions/asset/{id}` endpoint stays (used by the edit modal). All other cells already matched (Discussion interactive both places; History/Versions read-only; gallery Details/Related read-only).
  - Provisioned DBs (init SQL only runs on a fresh volume): `UPDATE options SET name='Assets Management' WHERE module='LIB' AND code='ASSETS';` and `UPDATE options SET name='Agents repository' WHERE module='LIB' AND code='AGENTS';`
  - Verified: build + tsc clean; Playwright smoke — edit modal shows all 6 tabs (Characterization singular; Related button "Save related assets"; Save hidden on Discussion/History/Versions, visible on editable tabs); gallery modal shows Details/Related/Discussion/History/Versions with **no** Permissions tab.
- Files affected: `ui/src/components/lib/AssetDetailModal.astro`, `ui/src/components/svelte/AssetDetailTabs.svelte`, `ui/src/components/lib/gallery/CatalogDetailModal.astro`, `ui/src/lib/{catalogDetail,assetSections}.ts`, `ui/src/pages/lib/agents.astro`, `ui/src/i18n/en.json`, `ui/src/i18n/es.json`, `db/sql/12-admin-insert.sql`, `api/app/lib/internal/models.py`, `api/tests/test_lib_versioning.py`
...
- **Frontend:** a 5th "Versions" tab on the shared read-only `CatalogDetailModal` (all three galleries — prompts/mcps/agents — inherit it), listing versions newest-first with date + who + change-type chip + a "Current" badge; each row expands to that version's characterization snapshot **rendered identically to the Details tab**. New `lib/versions.ts` (services + `mountVersions`, mirroring `lib/history.ts`) + `VersionsTimeline.astro` shell; `mountCatalogDetail` wires `mountVersions` alongside related/history. The Details-tab section renderer was extracted into an exported `renderCharacterizationSections()` in `catalogDetail.ts` and reused for the snapshot (copy buttons work via the dialog's existing delegated handler).
- Verified: 6 new backend tests (`test_lib_versioning.py`) on the Python 3.12 venv, full suite no new regressions; `bun run build` + `npx tsc --noEmit` clean (only pre-existing `advancedTable.ts` errors); Playwright smoke against a 2-version stub — tab lists both versions newest-first with the current badge/metadata, and expanding v1.0.0 vs v1.1.0 shows each version's distinct prompt template.
- Files affected: `api/app/lib/internal/{models,version_service}.py`, `api/app/lib/routes/assets.py`, `api/tests/test_lib_versioning.py`; `ui/src/components/lib/gallery/{CatalogDetailModal,VersionsTimeline}.astro`, `ui/src/lib/{catalogDetail,versions}.ts`, `ui/src/types/api.ts`, `ui/src/i18n/{en,es}.json`
## 2026-07-14 15:06 — feat(support): Support page, bug reporter, account-menu cleanup, mobile floating search

- **Account menu**: removed the dead "Account settings" entry (never had a distinct destination from `/profile`); "Support" now links to the new `/support` page.
- **Support page** (`/support`, account-menu-only like `/profile`): a contact card with a `mailto:` CTA (`PUBLIC_SUPPORT_EMAIL`, translated subject) and a report-a-bug card whose button opens the same bug reporter as the floating widget.
- **Bug reporter** — new `support` API domain: `bug_reports` table (`db/sql/81-support-ddl.sql`), `POST /api/support/reports` (any active user) + `GET /api/support/reports`/`{id}` (superuser-only), server-side size/count caps (400, never a bare 422), best-effort ops-email notification via the existing send-only mail module (`api/app/mail`) — a failed/disabled send never fails the request. Frontend: a global floating FAB (`BugReporter.svelte`, self-mounted via `BugReportWidget.astro` in `BaseLayout`) captures a screenshot (`html2canvas`, downscaled to JPEG), lets the user annotate it on a `<canvas>` (freehand pen / rectangle, undo/clear/retake), attach up to 3 extra images, describe the bug, and submit. The FAB itself is **red** (`bg-red-600`, not amber) and **hidden until the user explicitly clicks the `/support` page's "Report a bug" button** (merely visiting `/support` is not enough) — that click sets a `sessionStorage` flag activating the FAB for the rest of the tab session; `auth.ts`'s `clearToken()` clears the same flag on logout (and on a forced-logout 401), so a fresh login always starts with the FAB hidden again.
- **Mobile header search**: below `sm` the input collapses to an icon-only button; tapping it expands into a fixed floating card pinned below the header (`.search-palette-overlay`, mirrors `.notification-dropdown`'s mobile pattern) showing the same command-palette input + results. Desktop unchanged. **Follow-up fix 1:** collapsing `SearchBar`'s host to `shrink-0` below `sm` left nothing to flex-fill the row there, bunching hamburger/logo/search/bell/account together on the left with a dead gap on the right — `Header.astro`'s spacer div is now `flex-1 sm:hidden lg:block` (active below `sm` too, not just from `lg`) so the bell/account cluster sits flush against the right edge on phones again. **Follow-up fix 2:** the `sm`-to-`lg` tablet range (roughly 640-1024px) was cramped enough that the theme toggle overlapped the language pills and the brand name truncated to "Syna" — `ThemeSwitcher`/`LanguageSwitcher` in `Header.astro` moved from `hidden sm:flex` to `hidden lg:flex` (header-inline only from desktop), and `AccountMenu.astro`'s existing "mobile-only preferences" block (the same theme/language controls, previously `sm:hidden`) is now `lg:hidden` so it covers the freed-up tablet range too — same controls, just relocated into the account dropdown until there's room for them inline.
- Verified: 9 new backend tests (`api/tests/test_support_reports.py`) passing on the Python 3.12 venv, full suite shows no new regressions; `bun run build` + `npx tsc --noEmit` clean (pre-existing unrelated errors only); Playwright smoke tests at 390×844 and 1280×900 covering the account menu, support page, bug-report capture/annotate/submit round-trip, FAB presence across pages, the search icon/overlay/Escape flow, and the FAB's activation gating (hidden on `/support` and `/profile` until the "Report a bug" button is explicitly clicked, then persists across pages for that tab session, and clears back to hidden after a simulated logout).
- Files affected: `ui/src/components/core/header/{AccountMenu,Header,SearchBar}.astro`, `ui/src/components/core/BugReportWidget.astro`, `ui/src/components/svelte/{SearchPalette,BugReporter}.svelte`, `ui/src/layouts/BaseLayout.astro`, `ui/src/pages/support.astro`, `ui/src/lib/support.ts`, `ui/src/types/api.ts`, `ui/src/styles/globals.css`, `ui/src/i18n/{en,es}.json`, `ui/.env.development`, `ui/src/env.d.ts`, `ui/package.json`; `api/app/support/**`, `api/app/main.py`, `api/app/mail/templates/bug_report.html`, `api/tests/test_support_reports.py`, `db/sql/81-support-ddl.sql`
...
- **Caller-scoped repo listing:** `GET /api/assets/with-access` now returns only assets with an active, temporally-valid grant reaching the caller (USER/ROLE/TEAM/UNIT/PROJECT/PUBLIC), filtered BEFORE `skip`/`limit` so pages stay full. New additive `my_access` field ("MANAGE"|"VIEW"; superusers always MANAGE). The Postgres-only `array_agg`/`bool_or` summary aggregate was replaced with an equivalent Python-side reduction — same `is_active`-only semantics for the display fields, and the route is now SQLite-testable.
- **New service surface** (`api/app/lib/internal/permissions_service.py`): `_matching_perms` (one scope-bounded query), `assets_user_access`/`accessible_assets`/`user_asset_access` (effective level, MANAGE > VIEW), and `require_asset_manage` + `AssetAccessForbidden` — **the established per-asset write-guard pattern** (superuser bypass mirrors `require_privilege`; routes map the exception to 403 via a tiny `_ensure_manage` adapter).
- **12 guarded write routes** (module RBAC stays the outer layer): assets PUT/DELETE/`POST /{id}/versions`; characterizations POST/PUT/DELETE; asset_relations POST/PUT/DELETE (guard on the SOURCE asset); asset_permissions POST/PUT/DELETE (closes the escalation hole where a VIEW user could self-grant MANAGE). Propose/review/resubmit (own actions-based auth), galleries, single-asset GET and all reads unchanged. Non-superusers probing unknown assets via guarded writes get 403, not 404 (no existence disclosure).
- **Plain `POST /api/assets/` now auto-grants the creator USER/MANAGE** (mirrors propose) so the strict no-grant-hidden rule never orphans new assets. Seed note: asset 3 has no grant rows → now superuser-only until someone grants access.
- **UI:** `AssetWithAccessLevels.my_access` type; `assets.astro` maps `can_manage` and passes new opt-in `manageKey` prop through `DataTable → DataTableBody → DataTableActions` — VIEW rows hide edit/delete/detail-expand (+ separator dot), favorite star stays; default `null` keeps the other ~18 DataTable pages byte-identical. No new i18n strings.
- **Tests:** new `api/tests/test_lib_access_enforcement.py` (16 tests: effective-level math incl. all six scopes + temporal validity, caller-scoped listing + filter-first pagination, 403/2xx write-guard matrix, source-only relation guard, self-escalation block, create auto-grant); `test_assets_access.py` docstring refreshed. Full suite 206 passed on the 3.12 venv (same 8 pre-existing baseline failures).
- Files affected: `api/app/lib/internal/{permissions_service,models}.py`, `api/app/lib/routes/{assets,characterizations,asset_relations,asset_permissions}.py`, `api/tests/{test_lib_access_enforcement,test_assets_access}.py`, `ui/src/types/api.ts`, `ui/src/pages/lib/assets.astro`, `ui/src/components/table/DataTable.astro`, `ui/src/components/table/partials/{DataTableBody,DataTableActions}.astro`

## 2026-07-12 16:43 — feat(lib): characterization versioning on detail save (HU-LI09, versioning half)

- **What gets versioned is the CHARACTERIZATION set: saving the detail section of an existing asset creates a NEW VERSION; core-field edits stay in-place (plain PUT, unversioned).** Same-asset-row model (per the columns commit 496980c introduced): `assets.current_version` is bumped and the characterizations are snapshotted under the new `characterizations.version_label` (part of the PK `(asset, version_label, feature)`) — prior generations stay stored untouched as history. The user picks the change type at save time (Major / Minor / Patch radio pills in the modal footer, shown only when the detail/characterization section is being edited) and the matching digit of `X.Y.Z` bumps (major → X+1.0.0, minor → X.Y+1.0, patch → X.Y.Z+1). Each version save logs one `VERSIONING`/`FINISHED` action — the history timeline already localizes it as "created a new version", so the Activity tab shows versions with zero extra UI.
- **Backend:** new `api/app/lib/internal/version_service.py` (`bump_label` + transactional `create_version`) behind `POST /api/assets/{id}/versions` (403-gated like review/resubmit; 404 missing asset, 400 bad change_type/category, 409 label collision). `Asset` model gained `current_version`; `Characterization` gained `version_label` **as a PK column, fixing the live model/DB PK mismatch** left by 496980c. Every characterization read/write became current-version-scoped: the list endpoint filters `version_label == coalesce(assets.current_version,'1.0.0')` (so all UI consumers see exactly one generation), the `(asset, feature)`-keyed GET/PUT/DELETE resolve the asset's current label (URL contract unchanged), POST stamps new rows with it, and `modify_service.resubmit_asset`'s char upsert targets the current label (resubmit deliberately does NOT bump — review-loop edits stay in place; only the repo save flow versions). `PUT /api/assets/{id}` untouched (Constitution II). Propose still starts assets at `1.0.0`. No DDL — columns shipped in 496980c.
- **Frontend:** `AssetDetailModal.astro` shows a `vX.Y.Z` badge next to the title in edit mode; a **detail-section save** makes ONE `createAssetVersion` call (`change_type` + the tabs' full characterization snapshot via new `AssetDetailTabs.charSnapshot()`), then flushes relations/permissions (not versioned) via `tabs.flush(id, {skipChars:true})`; a **core (pencil) save** stays a plain in-place `updateAsset` PUT — no version, no picker. Create mode byte-identical. `/lib/assets` table gained a Version column. New i18n keys (`asset_modal.version`, `asset_detail_modal.version_saved`/`save_new_version`/`current_version`/`change_type_*`) in both `en.json` and `es.json`.
- **Versioning UX (follow-up commits on the same branch):** when the characterization section is being saved, the submit button reads **"Save new version"** (plain "Save" for create and core-only edits) and under the Major/Minor/Patch pills a **live preview** renders `v1.2.0 → v1.3.0` — old label struck-through/muted, arrow, new label with **the bumping digit highlighted** (indigo chip; major→1st, minor→2nd, patch→3rd) — recomputed on every pill change via a new client-side `bumpVersionLabel()` in `ui/src/lib/assets.ts` mirroring the backend's `bump_label` (display-only; the server stays authoritative).
- **Tests (Constitution III):** new `api/tests/test_lib_versioning.py` (17 tests: bump math incl. malformed labels, snapshot copy/edit/blank-drop semantics, action logging, guards, char CRUD current-version scoping, propose-at-1.0.0, route contract + openapi). Full lib suite 149 passed on the Python 3.12 venv; the 8 remaining repo failures (health/users/auth-DNS) pre-exist on develop with this change stashed.
- Files affected: `api/app/lib/internal/{models,version_service,modify_service}.py`, `api/app/lib/routes/{assets,characterizations}.py`, `api/tests/test_lib_versioning.py`, `ui/src/components/lib/AssetDetailModal.astro`, `ui/src/components/svelte/AssetDetailTabs.svelte`, `ui/src/{lib/assets.ts,types/api.ts,pages/lib/assets.astro,i18n/en.json,i18n/es.json}`, `db/CLAUDE.md`

## 2026-07-12 15:29 — feat(ui): header search palette (modules/options, client-side ranking)

- The header search bar was decorative only (a plain input + a non-functional ⌘K badge). It's now a working command-palette: typing filters the same modules/options that back the sidebar (`getNavigationData`/`readNavCache`, already cached from the sidebar's own fetch — no new API call), each result showing its breadcrumb (`Home › {Module} › {Option}`) and jumping straight to `option.path` on click/Enter.
- **Ranking** (`ui/src/lib/searchIndex.ts`, pure/no DOM deps): text relevance first (`fuzzyScore` — substring match scored by position+coverage, falling back to an in-order subsequence match so partial/typo'd queries still hit), then a flat **AI Library (`LIB`) module boost** so it outranks equally-relevant results from other modules, then a **usage boost** from how many times the user has opened that option. With an empty query the panel shows "Suggested" — the user's most-opened and most-recently-opened options (`suggestEntries`), LIB-boosted the same way.
- **Storage is client-side only, per the ask** — `localStorage["search_usage_v1"]` (open counts per option code) and `localStorage["search_recent_v1"]` (last 10 opened codes), written by `recordOptionUse()` on every click/Enter-select. No DB table, no API call — nothing server-side tracks what a user searches or opens.
- **UI**: `ui/src/components/svelte/SearchPalette.svelte` (new Svelte 5 island, manually `mount()`ed — same pattern as `NotificationBell.svelte`), self-mounted from `SearchBar.astro` (now a thin host, like `NotificationMenu.astro`). Wired ⌘K / Ctrl+K to focus+open globally, ↑/↓ to move the active row, Enter to navigate, Esc/outside-click to close. Module/option display names are resolved via `translate("modules.<code>")` / `translate("menu_options.<code>")` (same i18n keys the sidebar renders), so results stay in sync with the language switcher.
- Verified with a Playwright smoke test against the dev server (seeded `nav_cache`/usage via localStorage, no backend running): input mounts, ⌘/Ctrl+K focuses it, empty-query suggestions surface the seeded frequent/recent option, and a "pro" query ranks a LIB-module result above an equally-matching ADMIN one (module boost) with Enter navigating to it. `bun run build` passes.
- Files affected: `ui/src/lib/searchIndex.ts` (new), `ui/src/components/svelte/SearchPalette.svelte` (new), `ui/src/components/core/header/SearchBar.astro`, `ui/src/styles/globals.css`, `ui/src/i18n/en.json`, `ui/src/i18n/es.json`
...
- **Profile menu addition:** `AccountMenu.astro` gained a new `workItems` prop (separate from the existing static `menuItems`) rendered as its own "My Work" section, each item optionally profile-gated via a `profiles` allow-list. Since `AccountMenu` has no SSR user fetch (unlike the DB-driven sidebar), gating is done client-side in its existing mount script — after `getUser()` resolves, `[data-profiles]` items not matching the cached user's `profile` (or `is_superuser`) are hidden, and the whole "My Work" section hides itself if nothing in it is visible. The dropdown starts closed, so there's no visible flash before the script runs. `Header.astro` wires the two new items (`account_menu.review_requests` → `/lib/review_requests`, `account_menu.my_modifications` → `/lib/modifications`), both gated to `ADMINISTRATOR`/`ADMINISTRATIVE`/`REVIEWER` — the same profiles that hold `can_edit=TRUE` on `LIB.ASSETS` today.
- **Verify:** `npx tsc --noEmit` clean (only pre-existing `advancedTable.ts` errors); `bun run build` clean. Manual: a `COLLABORATOR` login shows no "My Work" section; `ADMINISTRATOR`/`ADMINISTRATIVE`/`REVIEWER` logins show both links, correctly routed.
- Files affected: `db/sql/12-admin-insert.sql`, `ui/src/components/core/header/AccountMenu.astro`, `ui/src/components/core/header/Header.astro`, `ui/src/i18n/en.json`, `ui/src/i18n/es.json`

## 2026-07-08 06:07 — fix(lib): dismissing a REVIEW/MODIFICATION notification no longer kills the assignment + new persistent "Review Requests" / "My Modifications" queues
- **Root cause:** the notification bell let a user "dismiss" any workflow notification, which inserts a `workflow_status=FINISHED` row — the exact same terminal state `review_asset()`/`resubmit_asset()` use to mean "already decided". Dismissing a REVIEW/MODIFICATION notification therefore didn't just hide the bell entry, it permanently revoked the reviewer's/proposer's ability to act on that asset (their guard queries filter on `ASSIGNED`/`NOTIFIED` only), with no way to recover it. This was also the *only* way to reach `/lib/review` or `/lib/modify` — there was no persistent, browsable place to find open assignments.
- **Fix — dismiss guard:** `actions_service.dismiss_notification` now raises `NotificationNotDismissible` (mapped to 400 in the route) unless the action's type is PUBLICATION/REJECTION (`DISMISSIBLE_TYPES`) — informational notifications with no further action. REVIEW/MODIFICATION must be resolved via review/resubmit instead. `NotificationBell.svelte` only renders the dismiss control for those two informational types now.
- **Fix — persistent queues:** new `GET /api/actions/reviews` / `GET /api/actions/modifications` (via a new `list_review_requests`/`list_pending_modifications` pair, refactored out of `list_notifications`'s now-shared `_list_open_actions` helper) back two new sidebar pages, `/lib/review_requests` ("Review Requests") and `/lib/modifications` ("My Modifications") — each lists the current user's open assignments of that type and links straight to `/lib/review?action={id}` / `/lib/modify?action={id}`, independent of whatever's been opened/dismissed in the bell. New `LIB.REVIEW_REQUESTS` / `LIB.MODIFICATIONS` sidebar options (seed-only, additive) granted to the same profiles that already hold `can_edit=TRUE` on `LIB.ASSETS` (ADMINISTRATOR/ADMINISTRATIVE/REVIEWER — the only profiles that can propose/review/resubmit today).
- **Verify:** extended `api/tests/test_lib_notifications.py` — dismiss now 400s for REVIEW/MODIFICATION (assignment stays open), still 200s for PUBLICATION/REJECTION; new `/reviews`/`/modifications` routes are type-scoped, auth-gated, and in OpenAPI.
- Files affected: `api/app/lib/internal/actions_service.py`, `api/app/lib/routes/actions.py`, `api/tests/test_lib_notifications.py`, `ui/src/lib/notifications.ts`, `ui/src/components/svelte/NotificationBell.svelte`, `ui/src/pages/lib/review_requests.astro` (new), `ui/src/pages/lib/modifications.astro` (new), `ui/src/i18n/en.json`, `ui/src/i18n/es.json`, `db/sql/12-admin-insert.sql`

## 2026-07-06 15:49 — fix(lib): characterization fields everywhere follow spec `sort_order` + control type + required, via a shared field builder
- **Order (backend):** `GET /api/characterizations/` now outer-joins `assets` → `specifications` (on the asset's category + feature) and orders by `(asset, coalesce(sort_order, 1e6), feature)` — within each asset, rows follow the configured order; features without a spec row go last. `coalesce` keeps NULL ordering deterministic on both Postgres and the SQLite test DB. Additive: no request/response keys changed, pagination preserved. **Order (frontend):** `getCharacterizationsByAsset` no longer re-sorts alphabetically (its `localeCompare` would have overridden any backend order) — it preserves the API order, so read-side consumers (ReviewAction, …) inherit it with no per-page changes.
- **New shared field builder `ui/src/lib/charFields.ts`** — one source of truth for characterization forms: `buildCharFieldDefs(category)` returns the fields **in spec `sort_order`**, each with the **control derived from the feature** (`select` when the feature has a `list` → options from `list_items` in the current language, `textarea` for long-form/RICH features, `input` otherwise — `RICH_FEATURES` now lives here), the **`required`** flag, and the feature's **FEAT_TYPE badge + description** for display. Feature/list lookups are cached; failures degrade to a plain text input.
- **Propose (`/lib/propose`) + Modify (`/lib/modify`) rebuilt on the builder:** list-backed features (e.g. LANGUAGE, MODE) now render as **selects** instead of free-text inputs; each field shows its FEAT_TYPE badge + description; required keeps the red `*` + submit blocking (selects included). Modify prefills from the asset's existing values (fallback: spec default). `AssetDetailTabs` (already type-driven + ordered) gained the **required `*`** marker on its labels.
- **Verify:** new `api/tests/test_lib_characterizations.py` (chars inserted alphabetically come back in spec `sort_order`, spec-less feature last); full lib+categories suite **140 passed** on the 3.12 venv; `bun run build` clean, `tsc` + `svelte-check` only the pre-existing `advancedTable.ts` errors.
- Files affected: `api/app/lib/routes/characterizations.py`, `api/tests/test_lib_characterizations.py` (new), `ui/src/lib/characterizations.ts`, `ui/src/lib/charFields.ts` (new), `ui/src/pages/lib/propose.astro`, `ui/src/components/svelte/ModifyAction.svelte`, `ui/src/components/svelte/AssetDetailTabs.svelte`
...
- **Fix:** both islands now guard on the asset's current status — `ReviewAction` renders its decision form only when `status == "PROPOSED"`, `ModifyAction` its edit form only when `status == "FEEDBACK"`. Otherwise they show a "no longer awaiting review/changes" notice + a **Back** button and hide the action buttons. The guard only trips when the status is positively known to be wrong (if the asset failed to load, it falls through and the backend 409 stays the last-resort guard). The header status pill already shows the live status. New i18n `review.not_actionable` / `review.back` + `modify.not_actionable` (en + es).
- **Spec `sort_order` wired end-to-end.** The `sort_order` column added to `specifications` (commit `85b5f29`) had no effect: the model didn't expose it and `GET /api/specifications/category/{code}` still ordered alphabetically by feature. Now `Specification`/`SpecificationCreate`/`SpecificationUpdate` expose `sort_order` and both specifications list routes `ORDER BY sort_order, feature`, so the Propose/Modify/asset-detail characterization inputs render in the configured order. Frontend `Specification*` types gained `sort_order?`. Contract tests added (round-trip, default 0, OpenAPI advertises it, and the by-category endpoint returns features in `sort_order`).
- **Verify:** `bun run build` clean; `tsc` + `svelte-check` show only the pre-existing `advancedTable.ts` errors. `api/tests/test_categories.py` passes (16), full categories+lib suite **139 passed** on the 3.12 venv. Manual: opening `/lib/review` (or `/lib/modify`) for a decided asset shows the notice + Back with **zero 409 toasts**; a genuinely PROPOSED/FEEDBACK asset still shows the full form with features in the configured order. Follow-up to the review/modify workflow shipped in PR #74 (now merged).
- Files affected: `ui/src/components/svelte/ReviewAction.svelte`, `ui/src/components/svelte/ModifyAction.svelte`, `ui/src/i18n/en.json`, `ui/src/i18n/es.json`, `api/app/taxo/internal/models.py`, `api/app/taxo/routes/specifications.py`, `api/tests/test_categories.py`, `ui/src/types/api.ts`

## 2026-07-02 17:19 — feat(lib): Review + Modify pages (HU-Review, HU-Modify) — the full propose→review→modify→re-review cycle + post-Propose gallery toast
- **Review workflow (HU-Review).** The assigned reviewer/administrator now opens a `REVIEW` notification from the header bell and is routed to a new **`/lib/review?action={id}`** page (instead of the old "coming soon" toast). The page shows the proposed asset + its characterizations and lets the reviewer leave feedback and **Approve / Request changes / Reject**. Opening it marks the notification seen (ASSIGNED→NOTIFIED, un-bolds the bell).
- **Decision backend.** `POST /api/assets/{id}/review` (gated `require_privilege("LIB","ASSETS", can_edit=True)`) → new `review_service.review_asset`: in one transaction it closes the reviewer's assignment (`REVIEW`/`FINISHED`), flips the asset status (`approve`→PUBLISHED, `reject`→REJECTED, `changes`→FEEDBACK), and notifies the **proposer** with a new `PUBLICATION`/`REJECTION`/`MODIFICATION` action (ASSIGNED) carrying the reviewer's feedback in `content` (Show Action renders it). Every transition is a new `actions` row — no new table, mirroring `propose_service`. 403 if the caller isn't the eligible/assigned reviewer; 409 if the asset is no longer `PROPOSED` (double-submit guard).
- **Modify / resubmit workflow (HU-Modify).** Clicking a `MODIFICATION` notification now routes the **proposer** to a new **`/lib/modify?action={id}`** page (replacing the "coming soon" toast). It shows the reviewer's feedback (read-only) + editable **name/description + characterizations** (prefilled from the asset's existing values, rich features as textareas, required specs enforced) and a **Resubmit** button. `POST /api/assets/{id}/resubmit` → new `modify_service.resubmit_asset`: one transaction that updates the asset + characterizations in place, closes the proposer's assignment (`MODIFICATION`/`FINISHED`), flips the status **`FEEDBACK`→`PROPOSED`**, and re-arms the **same original reviewer** with a fresh `REVIEW`/`ASSIGNED` — re-entering the HU-Review loop. 403 if the caller isn't the proposer holding the open MODIFICATION assignment; 409 if the asset isn't in `FEEDBACK`. **Doc-gap resolution:** the spec never states the resubmit status and there's no `MODIFIED` enum — it must return to `PROPOSED` because `review_service` guards `status == "PROPOSED"`, else re-review would be impossible.
- **Redirect + toast after Propose.** A successful Propose from a gallery CTA now **redirects back to that gallery** (`/lib/{category}?proposed=1`) and shows the confirmation toast there. **Fixed a deterministic bug** where that toast never appeared: `initCardGallery` ran before `installGlobalToast()`, so `window.showToast` was undefined at load — `catalogGallery.ts` now imports `showToast` directly from `@/lib/toast`. Wording updated to "…proposed and is now under review." (en+es). Direct visits (no locked category) keep the in-place confirm + reset.
- **Feedback rules:** review feedback required for Reject + Request changes (optional for Approve); modify requires a name + all required characterizations.
- **Stale-action guard.** Because every transition is a new `actions` row, the original `REVIEW`/`MODIFICATION` action stays `ASSIGNED` forever, so the Review/Modify page was still reachable for an already-decided asset (direct URL / back button / stale tab) and every click 409'd ("not awaiting review"). `ReviewAction`/`ModifyAction` now guard on the asset's live status — Review shows its form only when `status == PROPOSED`, Modify only when `status == FEEDBACK`; otherwise they show a "no longer awaiting review/changes" notice + Back and hide the action buttons (backend 409 stays the last-resort guard). New i18n `review.not_actionable`/`review.back` + `modify.not_actionable`.
- **Out of scope:** the review/modify cycle is uncapped by design (no iteration limit), matching the docs.
- **Verify:** `api/tests/test_lib_review.py` (10) + `api/tests/test_lib_modify.py` (9, incl. a full resubmit→approve→PUBLISHED loop) pass on the 3.12 venv; full lib suite **123 passed**. `bun run build` clean (`review` 6.65 kB, `modify` island bundled); `tsc` + `svelte-check` show only the pre-existing `advancedTable.ts` errors. **Deploy:** no schema change — the ASSET_STATUS/ACTION_TYPE/WORKFLOW_STATUS enums are already seeded and the REVIEWER profile ships from PR #69.
- Files affected: `api/app/lib/internal/review_service.py` (new), `api/app/lib/internal/modify_service.py` (new), `api/app/lib/internal/models.py`, `api/app/lib/routes/assets.py`, `api/tests/test_lib_review.py` (new), `api/tests/test_lib_modify.py` (new), `ui/src/lib/review.ts` (new), `ui/src/lib/modify.ts` (new), `ui/src/types/api.ts`, `ui/src/pages/lib/review.astro` (new), `ui/src/pages/lib/modify.astro` (new), `ui/src/components/svelte/ReviewAction.svelte` (new), `ui/src/components/svelte/ModifyAction.svelte` (new), `ui/src/components/svelte/NotificationBell.svelte`, `ui/src/pages/lib/propose.astro`, `ui/src/lib/catalogGallery.ts`, `ui/src/i18n/en.json`, `ui/src/i18n/es.json`, `memory/MEMORY.md`
