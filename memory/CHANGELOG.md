...
## 2026-07-03 20:02 — fix(lib): guard the Review/Modify pages against already-decided assets
...
- **Verify:** `bun run build` clean; `tsc` + `svelte-check` show only the pre-existing `advancedTable.ts` errors. Manual: opening `/lib/review` (or `/lib/modify`) for a decided asset shows the notice + Back with **zero 409 toasts**; a genuinely PROPOSED/FEEDBACK asset still shows the full form. Follow-up to the review/modify workflow shipped in PR #74 (now merged).
- Files affected: `ui/src/components/svelte/ReviewAction.svelte`, `ui/src/components/svelte/ModifyAction.svelte`, `ui/src/i18n/en.json`, `ui/src/i18n/es.json`
