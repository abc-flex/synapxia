...
## 2026-06-30 21:20 — feat(lib): customize the "Propose an asset" form (locked type, per-type title + characterization, working reviewers)
...
- **DB:** `REVIEWER` profile + its LIB/TAXO privileges added to `db/sql/12-admin-insert.sql`; idempotent `db/remediation/2026-06-28-reviewer-profile.sql` (`INSERT … ON CONFLICT DO NOTHING`) to apply on the already-initialized Neon DB (seed files only run on a fresh volume).
