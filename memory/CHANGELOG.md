...
- **Fix pass (PR #53 follow-up — same branch):** three voting defects.
  ...
  - *Vote-icon UX.* Vote controls now use **thumbs-up / thumbs-down** icons; active vs inactive reads as filled vs outline (`fill` toggle), and the title/aria-label flips to "Remove vote" when active so the un-vote toggle is discoverable. Added `gallery.vote_remove` (en/es) + a `translate()` helper in `i18nClient.ts` to keep the swapped labels localized. In the detail view (`CatalogDetailModal`) the vote bar moved up into the header beside the favorite star (was below the description).
