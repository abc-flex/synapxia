...
## 2026-06-15 06:54 — Responsive UI refresh: mobile drawer, card-view tables, modal, layout, dashboard + mobile header fix
...
- **Header (mobile overflow fix)**: hamburger → 44px; the top row no longer uses `justify-between` (which pushed the brand off-screen) — hamburger + brand are now left-aligned with `min-w-0` + `truncate`, and the account first-name is hidden <sm (avatar + chevron only) so the theme/lang/bell/account cluster fits 390px. Fixes the clipped "SynapxIA / Dashboard" brand and the runaway controls row seen on device.
