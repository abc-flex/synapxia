...
- [X] T039 [P] [US2] Parameterise `ui/src/components/svelte/Foro.svelte`: optional `api` prop (`{ getDiscussion, addComment, addQuestion, addAnswer, deleteParticipation }`, default = the current `lib/foro.ts` functions) and `idAttr` prop (default `"assetId"`); with `readonly`, only `getDiscussion` is required. Existing callers pass nothing and must behave identically — *first shipped as a read-only `fetchDiscussion` prop; after review (Discussion made interactive) it is the full `api` object as described*
