...
- **Follow-ups (same PR):**
  ...
  - **Read-only Permissions view on the gallery modal** — new `CatalogDetailModal` "Permissions" tab (covers prompt/mcp/agent) listing an asset's grants (target type · target · access level) via new `lib/assetPermissionsView.ts` (`mountPermissions`, cloned from `lib/history.ts`) + `PermissionsList.astro`, backed by the existing module-read-gated `GET /api/asset_permissions/asset/{id}`.
  - **Labels:** "Characterizations" tab → singular "Characterization"; sidebar `menu_options.assets` "Asset Repository" → **"Assets Management"**; `menu_options.agents` "Agent Index" → **"Agents repository"** (both i18n + `db/sql/12-admin-insert.sql` seed `options.name`; agents page title literals too).
  ...
  - Verified: build + tsc clean; Playwright smoke — edit modal shows all 6 tabs (Characterization singular), Save hidden on Discussion/History/Versions and visible on the editable tabs, Versions lists both versions; gallery modal shows the read-only Permissions tab with the grant.
- Files affected: `ui/src/components/lib/AssetDetailModal.astro`, `ui/src/components/svelte/AssetDetailTabs.svelte`, `ui/src/components/lib/gallery/CatalogDetailModal.astro`, `ui/src/components/lib/gallery/PermissionsList.astro`, `ui/src/lib/{catalogDetail,assetPermissionsView,assetSections}.ts`, `ui/src/pages/lib/agents.astro`, `ui/src/i18n/en.json`, `ui/src/i18n/es.json`, `db/sql/12-admin-insert.sql`
