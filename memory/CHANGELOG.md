...
## 2026-07-01 15:41 — feat(ui): adopt Svelte 5 for interactive islands (Show Action + Notifications bell), on Vite 8
...
- Both reuse the existing `lib/*` services + `translate()` i18n — **no backend change**. Live on every `BaseLayout` page (bell), validating Svelte islands coexisting with the layout's vanilla `<script>`s.
...
- Files affected: `ui/package.json`, `ui/astro.config.mjs`, `ui/svelte.config.js` (new), `ui/tailwind.config.cjs`, `ui/src/components/svelte/ShowAction.svelte` (new), `ui/src/components/svelte/NotificationBell.svelte` (new), `ui/src/pages/lib/show-action.astro`, `ui/src/components/core/header/NotificationMenu.astro`, `ui/src/lib/notifications.ts`, `ui/CLAUDE.md`, `memory/MEMORY.md`
