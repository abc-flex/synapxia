...
| Workflow notifications (HU-LI11) — header bell lists the user's open assignments (REVIEW/MODIFICATION/PUBLICATION/REJECTION) as latest-status `actions` threads (ASSIGNED bold → NOTIFIED dismissible → FINISHED gone); `list_notifications`/`mark_notified`/`dismiss_notification` + `/api/actions/notifications*`. **Click-through wired** (2026-07-01): PUBLICATION/REJECTION → Show Action; REVIEW/MODIFICATION → "coming soon" toast + mark-seen. | lib | ✅ done |
