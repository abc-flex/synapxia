...
- **Catalog detail modal (Prompt/MCP/Agent) now uses tabs** — `CatalogDetailModal` body split into **Details / Discussion / Activity** panels (Related lives under Details). `catalogDetail.ts` toggles panels, resets to Details on each open, and jumps to Discussion when the card's discuss shortcut (`data-foro-focus`) opens it. New `catalog_detail.tab_*` i18n (en/es).
- **Discussion (foro) reworked to be minimalist** — the composer moved to the **bottom** of the thread; the per-question **Answer** affordance is now a subtle link that reveals an inline answer box on click (was an always-visible button); lighter node borders; redundant in-section headings dropped (the tab labels them).
...
- Files affected: `ui/src/components/lib/gallery/{CatalogDetailModal,Foro,HistoryTimeline}.astro`, `ui/src/lib/{catalogDetail,foro}.ts`, `ui/src/components/lib/{PromptCard,McpCard}.astro`, `ui/src/pages/lib/{prompts,mcps,agents,assets}.astro`, `ui/src/i18n/{en,es}.json`
