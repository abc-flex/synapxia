...
- **Dark-mode toast fix:** the notification toast was unreadable in dark mode — the dark background was semi-transparent (`dark:bg-*-900/20`) so page content bled through. Made it opaque (`dark:bg-*-950` + `dark:text-*-200`) in both the live `ui/src/lib/toast.ts` (`VARIANT_CLASSES`) and the static `ui/src/components/Toast.astro` (`variantStyles`); light mode unchanged.
