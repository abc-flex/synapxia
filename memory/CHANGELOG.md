...
## 2026-06-19 22:57 — taxo/categories + taxo/features: expose missing DDL fields end-to-end
...
- No DDL change for either — both columns already exist.
- `tests/test_categories.py` now has 8 contract tests (field presence, value round-trip, optionality, OpenAPI schema for both `icon` and `list`) — all pass. Note: the 14 pre-existing suite errors come from a broken SQLite `create_all` fixture (FK resolution), unrelated to this change.
- Files affected: `api/app/taxo/internal/models.py`, `api/tests/test_categories.py`, `ui/src/types/api.ts`, `ui/src/types/category.ts`, `ui/src/pages/taxo/categories.astro`, `ui/src/pages/taxo/features.astro`, `ui/src/i18n/en.json`, `ui/src/i18n/es.json`
