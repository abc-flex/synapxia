...
- **Card-level discuss button:** added a chat-bubble button to the `GalleryCard` interaction bar (beside the like/dislike), shown whenever the card has a `detailModalId` (so no per-page wiring). Clicking it opens the detail view and (via a `data-foro-focus` flag on the synthetic opener) `mountForo` scrolls the discussion into view and focuses the comment composer. Optional `discussCount` prop on the card (server-side count plumbing is a later add). i18n `gallery.discuss` in en/es.
